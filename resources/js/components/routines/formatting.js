...
<div class="flex">
    ...
    <div class="routine--exercise--field">
        ...
        <input type="number" id="exercise-${exercise.id}-sets" name="exercises[${exercise.id}][sets]" value="${exercise.number_sets || '3'}" min="1" max="100">
    </div>

    <div class="routine--exercise--field">
        <label for="exercise-${exercise.id}-rest">Rest (seconds)</label>
        <input type="number" id="exercise-${exercise.id}-rest" name="exercises[${exercise.id}][rest]" value="${exercise.rest_seconds || '60'}" min="1">
    </div>
